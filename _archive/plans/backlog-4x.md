# LCARS Dashboard â€” 4.x Backlog (Completed Archive)

> Stable branch (`4.0`). Historical record of all completed 4.x work.
> Active TODO items are in `plans/backlog-4x.md`.
...
| `DONE` | Shipped â€” listed for context until next release |

---

## Backlog

### 4X-8 Â· Climate Panel Visual Refresh â€” `DONE` Â· Priority: MEDIUM Â· Size: L
...
| # | Issue | Severity | Fix |
|---|-------|----------|-----|
| 1 | Viewscreen background bleeds lavender from HA theme â€” not LCARS `#000` | Critical | Add explicit `background: var(--lcars-black, #000)` to `.climate-viewscreen` |
| 2 | Mode buttons are symmetric rounded pills â€” LCARS buttons have one flat side, one rounded | Major | First button: `border-radius: 1.5rem 0 0 1.5rem`, middle: `0`, last: `0 1.5rem 1.5rem 0` â€” connected strip |
| 3 | Setpoint `âˆ’`/`+` buttons are `border-radius: 50%` circles â€” circles are not LCARS vocabulary | Major | Replace with LCARS endcap pill buttons (`3rem Ã— 2.5rem`, one-sided radius) |
| 4 | SVG arc is thin 8px single-stroke â€” reads as generic gauge, not LCARS | Moderate | Segmented arc with `stroke-dasharray: 6 3` for discrete lit segments |
| 5 | Sensor dots (0.5rem circles) â€” LCARS uses short horizontal bars as status indicators | Minor | Replace with `2px Ã— 1rem` vertical mini-bars |
| 6 | Viewscreen corner brackets are 2px right-angles â€” should use mini-elbows and thickâ†’thin pattern | Moderate | Thicker brackets with mini-elbow corners, asymmetric border width (3px top/left, 1px bottom/right) |
| 7 | Font size on SVG temp readout (`42`) not mapped to LCARS 3-tier font scale | Minor | Map to `var(--lcars-font-size-title)` |

#### Visual Enhancements

**Temperature arc** (Wesley Approach A+B â€” segmented + dual-setpoint range):
- Replace thin stroke arc with `stroke-dasharray: 6 3` **segmented arc** â€” discrete lit segments like Main Engineering ring gauges
...
- In `heat_cool` mode: render comfort band between `target_temp_low` / `target_temp_high` â€” ice below, sunflower in band, butterscotch above
...
**Viewscreen treatment** â€” "Environmental Control Substation":
- Explicit `background: #000` â€” zero exceptions
- HVAC-action ambient radial glow: `radial-gradient` pseudo-element behind content at 12% opacity â€” butterscotch (heating), ice (cooling), transparent (idle/off). Transition 1s
- Scanline on mount: apply existing `lcars-scanline` keyframe via `firstUpdated()` â€” one-shot "power on" effect
- Asymmetric border: thick left/top, thin right/bottom per Bracer Jack Rule 2

**HVAC action dynamic feedback**:
- Frame border pulse: butterscotch breathe when heating, ice breathe when cooling â€” 2 new keyframes with `color-mix()` fallback via `@supports`
...
- Setpoint confirm flash: WAAPI `element.animate()` â€” scale(1.05) + gold text-shadow for 400ms, fire-and-forget (no class toggle cleanup)

**Mode strip** â€” connected LCARS button bar:
- First/last buttons bookend with rounded end, middle buttons flat â€” creates connected strip
- Traveling gold indicator bar under active mode â€” CSS `transform: translateX(var(--indicator-x))` driven by `updated()` lifecycle, 300ms ease-out transition
...
**Multi-zone awareness** (3 Nest thermostats â€” Boimler, Mariner, Tendi floors):
- Compact 3-zone summary strip below sensor column â€” floor label + current temp, each colored by HVAC action
...
- Colored by `getTempColor()` â€” instant trend visualization
- Fetch once on `connectedCallback`, refresh every 30 minutes

#### WCAG 2.2 Accessibility

Preserve and enhance existing accessibility:
- Existing `role="meter"` on arc, `role="radiogroup"` on strips, `focus-visible` outlines â€” all correct
...
- Maintain â‰¥24px target size on redesigned setpoint buttons (WCAG 2.5.8 AA)
...
| Frame border pulse | CSS keyframe Ã— 2 (heat/cool) | `--lcars-anim-pulse` | HVAC active |
...
- Viewscreen renders on pure black â€” no theme bleed-through
...
|-----------|------|---------|
| **4X-12** Floor/area hierarchy utilities | Extract `_getFloorAreaIds()` into shared `lcars-hierarchy-utils.js` â€” multi-zone awareness needs sibling areas on same floor | 5.x Floor â†’ Area â†’ Entity hierarchical layouts in every domain dashboard |

---

### 4X-9 Â· Pool & Spa Panel Visual Refresh â€” `DONE` Â· Priority: MEDIUM Â· Size: L
...
**Current state** (see screenshot): Pool/spa bodies are bordered rectangles with large temp + circular âˆ’/+ setpoint controls. Circuit buttons are flat-sided pills (correct shape). Chemistry sensors are inline dot+text rows. No water caustic effects, no pump telemetry, no WaterGuru integration. The panel feels utilitarian â€” functional but not "Cetacean Ops."
...
| # | Issue | Severity | Fix |
|---|-------|----------|-----|
| 1 | Setpoint `âˆ’`/`+` buttons are `border-radius: 50%` circles | Major | LCARS endcap pill buttons: decrement = rounded-left/flat-right, increment = flat-left/rounded-right |
...
| 3 | `.sensor-label` at `0.75rem` â€” 4th font size outside LCARS 3-tier system | Minor | Use `var(--lcars-font-size-data)` |
| 4 | `.pool-body-temp` at `2.5rem` â€” unmapped font size | Minor | Map to `var(--lcars-font-size-title)` |
| 5 | Water caustic shimmer + particles specced (Â§5.3/Â§8) but never implemented | Moderate | Implement per spec â€” caustic radial gradients + CSS particle drift |
| 6 | Inner viewscreen frame uses uniform border vs outer panel's thickâ†’thin | Moderate | Echo asymmetry: thick left/top, thin right/bottom |
| 7 | IntelliBrite lights are basic on/off toggles â€” spec Â§7 designed 22-color swatch strip | Major | Implement IntelliBrite color-mode selector with `screenlogic.set_color_mode` |

#### Visual Enhancements

**Water body viewscreens** â€” "Cetacean Ops observation windows" (Wesley):
- 3-layer CSS effect per viewscreen: (1) caustic shimmer â€” overlapping radial gradients at 6% opacity, 12s drift cycle, (2) surface ripple line â€” thin horizontal gradient band with sine-wave motion (`lcars-surface-ripple` 8s for pool, 5s for spa), (3) thermal gradient background â€” pool=ice at 4%, spa=butterscotch at 4%, intensifying to 8% when heating
...
**Chemistry visualization** â€” LCARS segmented bar indicators (Wesley):
- Replace dot+text rows with horizontal segmented bars per reading â€” 5â€“7 small rectangles (4px wide Ã— 12px tall), filled segments colored by threshold, unfilled at 30% opacity
...
- Non-color redundancy: shape indicators (â—/â–²/âœ•) alongside segments for WCAG
- Chemistry threshold function in `lcars-color-utils.js`:

| Metric | Optimal | Acceptable | Alert |
|--------|---------|------------|-------|
| pH | 7.2â€“7.6 | 7.0â€“7.8 | < 7.0 / > 7.8 |
| Free Chlorine | 1.0â€“3.0 ppm | 0.5â€“5.0 ppm | < 0.5 / > 5.0 |
| Total Alkalinity | 80â€“120 ppm | 60â€“150 ppm | < 60 / > 150 |
| Calcium Hardness | 200â€“400 ppm | 150â€“500 ppm | < 150 / > 500 |
| ORP | 650â€“750 mV | 550â€“800 mV | < 550 / > 800 |
| Salt | 2700â€“3400 ppm | 2500â€“3600 ppm | < 2500 / > 3600 |

**Pump telemetry** â€” EPS conduit power display (Wesley):
- Each pump gets mini telemetry readout when running: three segmented bars for RPM, Watts, GPM
- RPM: `--lcars-ice`, Watts: `--lcars-sunflower` (â†’butterscotch > 2000W, â†’tomato > 3000W), GPM: `--lcars-bluey`
...
**Freeze protection** â€” LCARS condition alert banner (Wesley):
...
**Temperature trend** â€” delta arrow indicator (Wesley):
- Below each body temp: direction arrow (â–²/â–¼/â€”) + delta + timeframe, e.g., `â–² +2Â° / 1HR`
...
- Stretch: 12-pip sparkline bar beneath temp (3px Ã— 12 segments, 48px wide)
...
|---------------|---------|------|---------------|
| `*_free_chlorine` | Free Chlorine | ppm | Chemistry â€” SANITIZER |
| `*_ph` | pH | â€” | Chemistry â€” BALANCE |
| `*_total_alkalinity` | Total Alkalinity | ppm | Chemistry â€” BALANCE |
| `*_calcium_hardness` | Calcium Hardness | ppm | Chemistry â€” BALANCE |
| `*_water_temperature` | Water Temperature | Â°F | Environmental (secondary to Pentair) |
| `*_chlorine_alert` | Chlorine Alert | â€” | Alert badge overlay on chlorine bar |
| `*_ph_alert` | pH Alert | â€” | Alert badge overlay on pH bar |
| `*_alkalinity_alert` | Alkalinity Alert | â€” | Alert badge overlay on alkalinity bar |
| `*_hardness_alert` | Hardness Alert | â€” | Alert badge overlay on hardness bar |
...
| A6 | Chemistry threshold colors need non-color redundancy | Add shape indicators (â—/â–²/âœ•) per threshold state |
...
| Water particles (6 per body) | CSS keyframe | 6â€“10s each | Always |
...
**Acceptance criteria**:
- All 7 LCARS compliance violations fixed
- Water body viewscreens have caustic shimmer, surface ripple, and thermal gradient â€” feel like "observation windows"
- Pool = cool blue, Spa = warm amber â€” visually distinct at a glance
...
| **4X-16** `<lcars-segmented-bar>` component | Shared threshold-colored bar for chemistry readings (pH, chlorine, alkalinity, RPM, watts) | Reusable across every 5.x dashboard (power load bars, COâ‚‚ bars, signal bars) |

---

### 4X-10 Â· Life Support / Environmental Systems Panel â€” `DONE` Â· Priority: MEDIUM Â· Size: XL

**GitHub Issue**: [#10](https://github.com/htiel/LCARS-lovelace-dashboard/issues/10)
**Spec**: None yet â€” see design notes below

A new room-level composite panel (`<lcars-lifesupport-panel>`) that aggregates **all environmental entities for a single area/room** into one unified view: thermostat (climate control), air purifier (atmospheric processing), and standalone temperature/humidity sensors (ambient monitoring). Think "Environmental Control Substation" â€” one console, everything that keeps you breathing and comfortable.

> *"Think of this as everything that keeps me breathing, and comfortable in my room."*

**Key constraint**: The atmoscrubber cylinder visualization is preserved exactly as-is. This panel does NOT replace the existing `<lcars-climate-panel>` or `<lcars-environment-panel>` â€” it's an additional panel type that users can configure for specific areas.

#### Why a New Panel

Current architecture renders panels per-device. SwitchBot temp/humidity sensors have no parent device panel â€” they're orphaned entities. This panel solves three problems:
...
- Master Bedroom: Awair Element (COâ‚‚/VOC/PM2.5/temp/humidity) + VeSync purifier
...
LifeSupportGroup {
  ...
  climateDevice â†’ climate.*, thermostat sensors

  // Substation B: Atmoscrubber (0 or 1 air purifier)
  environmentDevice â†’ fan, AQ sensors, controls

  // Ambient: Standalone sensors not tied to climate or environment devices
  ambientSensors â†’ SwitchBot meters, additional temp/humidity entities
}
```
...
The panel uses a horizontal split when both climate and atmoscrubber substations are present â€” two dedicated zones side by side, like adjacent stations on an Engineering console. When only one substation is present, it expands to fill the full width.
...
This means zero duplication â€” child panels get their own shadow DOM/styles/lifecycle, and any future improvements to the individual panels automatically flow into the composite view.

#### Graceful Degradation â€” Four Configurations
...
| **Sensors only**: standalone temp/humidity only | Children's room with SwitchBot | "Sensor hero" layout â€” large centered temp + humidity + battery + sparklines |

**Sensor hero layout** (sensors-only rooms): Temperature displayed at title tier (3.5rem) as the visual anchor, colored by comfort zone via existing `getTempColor()`. Humidity and battery below in data tier. No arc, no cylinder â€” just the essential readings, LCARS-styled.
...
- Same AQIâ†’HSL hue coloring on border + particles
...
```
AMBIENT SENSORS
â— SWITCHBOT  TEMP 71.8Â°F  HUM 47%  â–ªâ–ªâ–ªâ–ªâ–ªâ–ªâ–ªâ–ªâ–‘â–‘ 82%
```
...
```
TEMP   72.1Â°   â—â—â—‹    â† 3 sources, 2 agree within Â±1Â°
```

#### Adaptive Sparkline Tray

Single bottom row rendering 2â€“6 sparklines depending on available data:
...
| 5 | COâ‚‚ | `--lcars-sunflower` | If Awair/SCD40 present |
...
**Life Support Efficiency (LSE) score** â€” weighted comfort composite:
$$\text{LSE} = 0.40 \times C_{\text{temp}} + 0.30 \times C_{\text{humidity}} + 0.30 \times C_{\text{air}}$$
Each component scored 0â€“100 via trapezoidal membership function. Displayed as header badge (`LSE 94%`) in dynamic color. Drives cylinder particle speed (below 60% = scrubbers working harder, above 90% = gentle drift). Degrades gracefully when components unavailable.

**Room ambient tint** â€” CSS `background-image: radial-gradient()` overlay at â‰¤ 0.06 opacity. Butterscotch when heating, ice when cooling, faint green when all-nominal, faint red when AQI > 150. Uses `@property --ambient-tint` for smooth CSS transitions. Subtle enough to be felt not seen.

**Dual-Core Column** (alternative to side-by-side) â€” the cylinder gains a temperature gradient band along its right edge, functioning as a thermometer. Fill height = current temp in range, fill color tracks HVAC action. Cylinder body + particles still track AQI. One visualization, two systems. Requires Geordi sign-off on proportions.

**Atmospheric circulation indicator** â€” CSS-only rotating sweep of light around the cylinder perimeter (conic-gradient pseudo-element, 8s rotation). Speed increases with fan percentage. Invisible when fan is off.

**Mini sensor mesh** â€” tiny row of 3â€“5 tiles showing other rooms' temp/humidity at a glance. "Nearby sections" context â€” the bridge officer glances at adjacent decks.
...
- 4X-8 (Climate Panel Visual Refresh) â€” climate substation benefits from the refreshed arc and mode strip
- Panel Extraction Architecture (`specs/LCARS-PANEL-EXTRACTION-ARCHITECTURE/`) â€” the `frameless` child panel composition pattern needs `LcarsBasePanel` to support suppressing its own frame
...
- Sparkline tray adapts to available metrics (2â€“6 sparklines)
...
| Prep Item | What | Enables |
|-----------|------|---------|
| **4X-13** Entity query utility (`lcars-entity-query.js`) | Extract `_getAreaEntities/_groupEntities` into shared module with `queryEntities(hass, { areaIds?, domains?, deviceClasses? })` | Every 5.x dashboard â€” cross-area queries become a parameter change, not an architecture change |
...
| **4X-17** `classifyArea()` function | Area-level panel detection returning `Set<PANEL_TYPE_*>` â€” determines composite panels (life_support, illumination) per area | 5.x dashboard-level classification â€” same pattern at broader scope |
...
| **4X-19** `LcarsBasePanel` frame-mode property | `frame-mode` attribute: `standard` (default), `nested` (header only, no borders), `header-only` (compact) â€” per Geordi's review | 5.x dashboard layouts embed panels without double-framing |
| **4X-21** Panel dispatch registry | Replace `_renderDevicePanel()` switch with `Map<panelType, factory>` â€” factory function pattern per Geordi | 5.x dashboards register panel types without touching orchestrator code |

---

### 4X-11 Â· Illumination Control Panel â€” `DONE` Â· Priority: MEDIUM Â· Size: L
...
- No brightness overview at a glance â€” you can't tell which lights are dim vs full
...
| `switch` | (lighting circuits) | Toggle on/off â€” no dimming |
...
â•‘ ILLUMINATION CONTROL â€” MASTER BEDROOM   3/5  ON  â•‘
â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£
â•‘                                                   â•‘
â•‘  â— CEILING     â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â–‘â–‘â–‘â–‘â–‘  80%  â•‘
â•‘  â— BEDSIDE L   â”â”â”â”â”â”â”â”â”â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘  35%  â•‘
â•‘  â— BEDSIDE R   â”â”â”â”â”â”â”â”â”â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘  35%  â•‘
â•‘  â—‹ CLOSET      â”â”â”â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘  OFF  â•‘
â•‘  â—‹ BATHROOM    â”â”â”â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘  OFF  â•‘
â•‘                                                   â•‘
â•‘  â”€â”€â”€â”€â”€â”€â”€ SCENES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ â•‘
â•‘  [ BRIGHT ]  [ RELAX ]  [ MOVIE ]  [ SLEEP ]    â•‘
â•‘                                                   â•‘
â•‘  â”€â”€â”€â”€â”€â”€â”€ CIRCUITS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ â•‘
â•‘  â— PORCH LIGHT                              ON   â•‘
â•‘  â—‹ ENTRY LIGHT                              OFF  â•‘
â•‘                                                   â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
```

**Sections** (rendered in order):
1. **Dimmable lights** â€” full-width brightness bars with toggle + slider
2. **Scenes** â€” horizontal strip of scene activation buttons (LCARS endcap pills)
3. **Switch circuits** â€” simple on/off rows for non-dimmable lighting switches

**Badge**: `3/5 ON` â€” active count / total count

**Frame color**: `var(--lcars-sunflower)` â€” warm light, distinguishes from other panels
...
- Background: fill bar proportional to brightness (0â€“100%), colored by color temperature if available (warm amber â†’ cool white)
- Click the bar â†’ toggle light on/off
- Click the percentage â†’ open brightness slider popover (or inline expand)
- Long-press / right-click â†’ HA more-info dialog
...
- No color picker wheel for v1 â€” just temperature-aware bar coloring
...
| 1â€“2 dimmable lights, no scenes | Compact panel: brightness bars only |
| Switches only, no dimmable lights | Switch list with simple toggles |
| Single light | Minimal: one bar, no dividers |

#### Dependencies

- Panel Extraction Architecture â€” extends `LcarsBasePanel`
- `lcars-entity-utils.js` â€” needs new `PANEL_TYPE_ILLUMINATION` constant and detector
- `lcars-homepage-card.js` â€” needs dispatch case in `_renderDevicePanel()`
...
| **4X-20** `<lcars-summary-badge>` component | Reusable status badge: `3/5 ON`, `1847 W` â€” `role="status"` + `aria-live="polite"` | 5.x dashboard summary bars composed from badge components |
...
### 4X-12 Â· Floor/Area Hierarchy Utilities â€” `DONE` Â· Priority: MEDIUM Â· Size: S
...
**Enables**: 5.x Floor â†’ Area â†’ Entity hierarchical layouts in every domain dashboard (5X-2.2 through 5X-2.5).

**Geordi note**: Pure data utility â€” no render imports. Include floor sort order.
...
### 4X-13 Â· Entity Query Utility â€” `DONE` Â· Priority: HIGH Â· Size: M
...
**Fits into**: 4X-10 (Life Support needs multi-device area query) â€” implement during 4X-10.
...
### 4X-14 Â· Cross-Device Entity Injection (`linkedEntities`) â€” `DONE` Â· Priority: MEDIUM Â· Size: S

**GitHub Issue**: [#14](https://github.com/htiel/LCARS-lovelace-dashboard/issues/14)
**Label**: `5x-prep`

Add `linkedEntities` property to `LcarsBasePanel` â€” an array of external entities merged into the panel's working set. Merge happens in computed getter, not `updated()`.

**Fits into**: 4X-9 (WaterGuru cross-device pattern) â€” implement during 4X-9.

**Enables**: 5.x panels aggregating entities from many devices/areas.

**Geordi note**: Linked entities must carry `_linked: true` provenance flag for optional source indicators. Same visual treatment as native entities â€” no `aria-hidden`.
...
### 4X-15 Â· Panel Data Model: `entities` Collection â€” `DONE` Â· Priority: HIGH Â· Size: M
...
**Fits into**: 4X-10 (Life Support spans multiple devices) â€” implement during 4X-10.
...
### 4X-16 Â· `<lcars-segmented-bar>` Shared Component â€” `DONE` Â· Priority: MEDIUM Â· Size: S
...
**Fits into**: 4X-9 (chemistry bars, pump telemetry) â€” implement during 4X-9.

**Enables**: Reusable across every 5.x dashboard â€” power load bars, COâ‚‚ levels, signal strength.

**Geordi rules** (mandatory):
- **NO gradients** â€” Bracer Jack Rule 1. Solid-color adjacent blocks only
...
### 4X-17 Â· `classifyArea()` Function â€” `DONE` Â· Priority: HIGH Â· Size: M
...
**Fits into**: 4X-10 and 4X-11 â€” implement during 4X-10.

**Enables**: 5.x dashboard-level classification at broader scope.

**Risk**: Medium â€” changes render dispatch path. Entities claimed by area-level panels must not double-render in device panels. Mitigation: area-level panels return consumed entity IDs.
...
### 4X-18 Â· Domain/Device_Class Filter Predicates â€” `DONE` Â· Priority: MEDIUM Â· Size: S
...
- `createDomainFilter(domains)` â€” returns predicate `(entry) => domains.has(entry.domain)`
- `createDeviceClassFilter(classes)` â€” returns predicate
- `createCompositeFilter(...predicates)` â€” OR composition
...
### 4X-19 Â· `LcarsBasePanel` Frame-Mode Property â€” `DONE` Â· Priority: HIGH Â· Size: S
...
- **`nested`**: No outer border, keeps header bar (name + line + code) â€” preserves panel identity within parent frame
- **`header-only`**: Header bar only, no borders, no corner brackets â€” tightly packed layouts

**Fits into**: 4X-10 (climate/environment panels embedded as substations in Life Support) â€” implement during 4X-10.

**Enables**: 5.x dashboard layouts embedding existing panels without double-framing (5X-2.2 through 5X-2.5).

**Geordi note**: Rename from Wesley's `frameless` to `frame-mode`. The frame IS LCARS visual grammar â€” preserving semantic structure and header maintains identity. `role="region"` + `aria-label` persist regardless of frame mode.
...
### 4X-20 Â· `<lcars-summary-badge>` Shared Component â€” `DONE` Â· Priority: MEDIUM Â· Size: S
...
- No borders, no background, no shadows â€” text-only
...
### 4X-21 Â· Panel Dispatch Registry â€” `DONE` Â· Priority: MEDIUM Â· Size: S
...
**Fits into**: 4X-10 and 4X-11 (both add new panel types) â€” implement during 4X-10.
...
### 4X-22 Â· `load_dashboard.py` Parametric Registration â€” `DONE` Â· Priority: HIGH Â· Size: S
...
**Enables**: 5X-2.0 Dashboard Registration Framework â€” a loop calling `_register_single_dashboard()` N times. The 5.x work becomes config + YAML files, not plumbing.
...
### 4X-23 Â· Config Flow Options Schema Prep â€” `DONE` Â· Priority: LOW Â· Size: S

**GitHub Issue**: [#23](https://github.com/htiel/LCARS-lovelace-dashboard/issues/23)
**Label**: `5x-prep`

Add `dashboards` key to `config_entry.options` in `config_flow.py` with default `["habitat"]`. Not exposed in UI yet â€” forward-compatible schema only.

**Enables**: 5X-2.6 Dashboard Config Flow â€” adding checkboxes in 5.x is a UI change, not a data model change.
...
### 4X-24 Â· Dashboard Identity CSS Custom Properties â€” `DONE` Â· Priority: LOW Â· Size: S
...
**Geordi note**: 2â€“3 hue families per dashboard. Within Bracer Jack's safe zone. Each dashboard sets `--lcars-active-dash` to its identity color.
...
### 4X-25 Â· Shared Focus Style Mixin â€” `DONE` Â· Priority: LOW Â· Size: S
...
**Geordi note**: Named "seraph" â€” the focus halo that guides attention. Required for WCAG 2.4.7 + 2.4.13.
...
Carried forward from 4.x archive. Creative enhancement ideas â€” not committed to a version.

### Environment Panel
- CSS `@property --aqi-hue` with `syntax: '<angle>'` â€” GPU-accelerated color interpolation for cylinder gradient
- AQI "alert condition" escalation: 0â€“50 "All clear" (green) â†’ 150+ "Red alert" (frame pulses red)
- Optional Web Audio API ambient hum tied to fan speed. *Worf: AudioContext requires user gesture.*

### Floor Navigation
- View Transition API (`document.startViewTransition()`) â€” turbolift-style vertical slide between floors. Wrap in `prefers-reduced-motion`.

### Web API Adoption

| API | Baseline | Use | Priority |
|-----|----------|-----|----------|
| Popover API | Widely Available | Replace `lcars-popup.js` â€” zero-JS top layer, light-dismiss, focus trapping | HIGH |
...
| `text-wrap: balance` | Widely Available | Panel headers â€” prevent orphan words | LOW |
...
### 4X-31 - Environmental + air purifier panels clip in consolidated room view - `DONE v4.18.9 ï¿½ REOPENED as 4X-46` - Priority: HIGH - Size: M - WSJF: 3.33
...
## Completed Items (v4.22.0)

### P1-P3: 43 sensor classification, formatting, and camera bugs - `DONE v4.22.0-rc.1 through rc.3`

**GitHub Issue**: [#58](https://github.com/htiel/LCARS-lovelace-dashboard/issues/58)

Sensor entity classification overhaul, formatNumber()/formatSensorValue() utilities, camera panel refresh improvements.

---

### P4: 10 power naming and circuit label bugs - `DONE v4.22.0-rc.4`

**GitHub Issue**: [#59](https://github.com/htiel/LCARS-lovelace-dashboard/issues/59)

Power panel naming conventions, circuit label formatting, EcoFlow/Emporia integration fixes.

---

### P5: 8 tactical, alarm, garage door, and safety bugs - `DONE v4.22.0-rc.5`

**GitHub Issue**: [#60](https://github.com/htiel/LCARS-lovelace-dashboard/issues/60)

Alarm panel state colors, garage door entity routing, safety sensor classification.

---

### P6: 3 site crawl bugs - `DONE v4.22.0-rc.6`

**GitHub Issue**: [#61](https://github.com/htiel/LCARS-lovelace-dashboard/issues/61)

Tactical camera, navigation, and crawl-discovered rendering issues.

---

### P6b-P9: 17 formatting, offline, media, a11y, security fixes - `DONE v4.22.0-rc.8`

**GitHub Issue**: [#62](https://github.com/htiel/LCARS-lovelace-dashboard/issues/62)

Shared formatting utilities, weather/irrigation offline degradation, media compaction, accessibility polish, security hardening.

---

### Code Review: 2 Critical, 5 High, 4 Medium fixes - `DONE v4.22.0-rc.9`

**GitHub Issues**: [#63](https://github.com/htiel/LCARS-lovelace-dashboard/issues/63)-[#69](https://github.com/htiel/LCARS-lovelace-dashboard/issues/69)

- C1: Async I/O rewrite for process_yaml.py (blocking to executor)
- C2: _safe_json_loads guard on all 12 WS json.loads sites
- H1: Notification WS type corrected
- H2: Reload crash on empty YAML guarded
- H3: File handle leak eliminated (read/write helpers)
- H4: Per-file asyncio.Lock for YAML write race prevention
- H5: Climate debouncer cancelled on disconnectedCallback
- M1: Variable shadowing in process_yaml fixed
- M2: Entities list type validation added
- M4: Dead alarm timer cleanup removed from homepage card
- M5: Notification FIFO eviction (max 100)

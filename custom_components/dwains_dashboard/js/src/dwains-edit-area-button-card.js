...
Promise.race(waitForHelpers).then(async () => {
  ...
  class LcarsEditAreaButtonCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'dwains_dashboard/area_button/set', ...this._getFormData() });
  ...
  if (!customElements.get('dwains-edit-area-button-card')) {
    customElements.define('dwains-edit-area-button-card', LcarsEditAreaButtonCard);
  }
});

/**
 * LCARS Edit Device Button Card — Editor for device button appearance
 */
...
Promise.race(waitForHelpers).then(async () => {
  ...
  class LcarsEditDeviceButtonCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'dwains_dashboard/device_button/set', ...this._getFormData() });
    ...
    _getFormData() {
      ...
      return { device: inputs[0]?.value, name: inputs[1]?.value, icon: inputs[2]?.value };
    }
  ...
  if (!customElements.get('dwains-edit-device-button-card')) {
    customElements.define('dwains-edit-device-button-card', LcarsEditDeviceButtonCard);
  }
});

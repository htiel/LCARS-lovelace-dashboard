...
Promise.race(waitForHelpers).then(async () => {
  ...
  class LcarsEditDeviceCardCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'dwains_dashboard/device_card/set', ...this._getFormData() });
  ...
  if (!customElements.get('dwains-edit-device-card-card')) {
    customElements.define('dwains-edit-device-card-card', LcarsEditDeviceCardCard);
  }
});

...
Promise.race(waitForHelpers).then(async () => {
  ...
  class LcarsEditDevicePopupCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'dwains_dashboard/device_popup/set', ...this._getFormData() });
  ...
  if (!customElements.get('dwains-edit-device-popup-card')) {
    customElements.define('dwains-edit-device-popup-card', LcarsEditDevicePopupCard);
  }
});

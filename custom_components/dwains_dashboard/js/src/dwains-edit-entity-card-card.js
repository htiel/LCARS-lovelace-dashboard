/**
 * LCARS Edit Entity Card Card — Editor for entity card configuration
 */
...
Promise.race(waitForHelpers).then(async () => {
  ...
  class LcarsEditEntityCardCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'dwains_dashboard/entity_card/set', ...this._getFormData() });
    ...
    _getFormData() {
      ...
      return { entity: inputs[0]?.value, name: inputs[1]?.value };
    }
  ...
  if (!customElements.get('dwains-edit-entity-card-card')) {
    customElements.define('dwains-edit-entity-card-card', LcarsEditEntityCardCard);
  }
});

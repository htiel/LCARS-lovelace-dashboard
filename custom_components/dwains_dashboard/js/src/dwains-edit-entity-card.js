/**
 * LCARS Edit Entity Card — Editor for single entity settings
 */
...
Promise.race(waitForHelpers).then(async () => {
  ...
  class LcarsEditEntityCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'dwains_dashboard/entity/set', ...this._getFormData() });
    ...
    _getFormData() {
      ...
      return { entity: inputs[0]?.value, icon: inputs[1]?.value, name: inputs[2]?.value };
    }
  ...
  if (!customElements.get('dwains-edit-entity-card')) {
    customElements.define('dwains-edit-entity-card', LcarsEditEntityCard);
  }
});

/**
 * LCARS Edit Homepage Header Card — Editor for homepage header configuration
 */
...
Promise.race(waitForHelpers).then(async () => {
  ...
  class LcarsEditHomepageHeaderCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'dwains_dashboard/homepage_header/set', ...this._getFormData() });
    ...
    _getFormData() {
      ...
      return { title: inputs[0]?.value, subtitle: inputs[1]?.value };
    }
  ...
  if (!customElements.get('dwains-edit-homepage-header-card')) {
    customElements.define('dwains-edit-homepage-header-card', LcarsEditHomepageHeaderCard);
  }
});

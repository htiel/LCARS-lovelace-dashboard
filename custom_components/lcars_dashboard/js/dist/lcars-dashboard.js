...
`;class l extends r.WF{static get properties(){return{_hass:{type:Object},_config:{type:Object},_order:{type:Array},_columnOverrides:{type:Object},_selected:{type:String}}}set hass(e){this._hass=e}setConfig(e){this._config=e;const t=e?.panels||[];this._order=t.map(e=>e.panelId),this._columnOverrides={...e?.column_overrides||{}},this._selected=e?.panel_id||t[0]?.panelId||""}_getPanelInfo(e){return(this._config?.panels||[]).find(t=>t.panelId===e)}_isIllumination(e){const t=this._getPanelInfo(e);return t?.panelType===n.sv}_getColumn(e){if(this._isIllumination(e))return"full";const t=this._getPanelInfo(e);return this._columnOverrides[e]||this._columnOverrides[t?.panelType]||n.uC[t?.panelType]||"left"}_getLabel(e){const t=this._getPanelInfo(e);return t?t.label||t.panelType.replace(/_/g," "):e}_select(e){this._selected=e}_moveUp(){const e=this._getColumn(this._selected),t=this._order.filter(t=>this._getColumn(t)===e),a=t.indexOf(this._selected);if(a<=0)return;const r=this._order.indexOf(this._selected),i=this._order.indexOf(t[a-1]),s=[...this._order];[s[i],s[r]]=[s[r],s[i]],this._order=s}_moveDown(){const e=this._getColumn(this._selected),t=this._order.filter(t=>this._getColumn(t)===e),a=t.indexOf(this._selected);if(a<0||a>=t.length-1)return;const r=this._order.indexOf(this._selected),i=this._order.indexOf(t[a+1]),s=[...this._order];[s[r],s[i]]=[s[i],s[r]],this._order=s}_moveLeft(){this._isIllumination(this._selected)||"left"!==this._getColumn(this._selected)&&(this._columnOverrides={...this._columnOverrides,[this._selected]:"left"})}_moveRight(){this._isIllumination(this._selected)||"right"!==this._getColumn(this._selected)&&(this._columnOverrides={...this._columnOverrides,[this._selected]:"right"})}async _save(){if(this._hass&&this._config?.area_id)try{await this._hass.callWS({type:"lcars_dashboard/panel_order/set",area_id:this._config.area_id,panel_order:JSON.stringify(this._order)});const e={};for(const[t,a]of Object.entries(this._columnOverrides)){const r=this._getPanelInfo(t);a!==(n.uC[r?.panelType]||"left")&&(e[t]=a)}await this._hass.callWS({type:"lcars_dashboard/panel_column/set",area_id:this._config.area_id,panel_columns:JSON.stringify(e)}),(0,s.rC)("lcars_dashboard_reload")}catch(e){console.error("LCARS Edit: Panel layout save failed",e)}}async _reset(){if(this._hass&&this._config?.area_id)try{await this._hass.callWS({type:"lcars_dashboard/panel_order/set",area_id:this._config.area_id,panel_order:JSON.stringify([])}),await this._hass.callWS({type:"lcars_dashboard/panel_column/set",area_id:this._config.area_id,panel_columns:JSON.stringify({})});const e=this._config?.panels||[];this._order=e.map(e=>e.panelId),this._columnOverrides={},(0,s.rC)("lcars_dashboard_reload")}catch(e){console.error("LCARS Edit: Panel layout reset failed",e)}}static get styles(){return[i.Bx,o]}_renderPanelItem(e,t){const a=e===this._selected,i=this._isIllumination(e),s=this._getPanelInfo(e),n=s?.panelType?.replace(/_/g," ")||"",o=this._getLabel(e),l=s?.deviceId&&o.toLowerCase()!==n.toLowerCase();return r.qy`
    ...
    `}getCardSize(){return 5}}customElements.get("lcars-edit-panel-order-card")||customElements.define("lcars-edit-panel-order-card",l)},5316(e,t,a){var r=a(7349),i=a(8851),s=a(2622),n=a(7597),o=a(6930),l=a(4867),c=a(9411);a(6940),a(6564),new Set(["battery","power","energy","voltage","current"]);const d=/ups|battery_charge|battery_runtime|battery_voltage/i,p=/grid|mains|mainsfromgrid|main.*load|total.*power|vueg3.*main|shelly.*total|3em.*total/i,u=new Set(["ecoflow_cloud","nut","victron","tesla_powerwall","solaredge"]),m=new Set(["wallbox","insteon","blink","simplisafe","tile","switchbot","unifiprotect","unifi","mobile_app","nest_protect"]),h=/motion.sensor|remote|phone|tablet|watch|tile|tag|lock|camera|protect|switch.?bot|wallbox|vilya|charger|thermostat|meter|doorbell/i;class f extends r.WF{static get properties(){return{hass:{type:Object},_config:{type:Object},filter:{type:String}}}constructor(){super(),this._hass=null,this._config={},this.filter="all",this._entityCache=new Map,this._onFilter=e=>{this.filter=e.detail.filter}}connectedCallback(){super.connectedCallback(),i.o6.addEventListener("lcars-eng-filter",this._onFilter)}disconnectedCallback(){super.disconnectedCallback(),i.o6.removeEventListener("lcars-eng-filter",this._onFilter)}setConfig(e){this._config=e||{}}set hass(e){const t=this._hass;this._hass=e,e&&t!==e&&(this._entityCache.clear(),this.requestUpdate("hass",t))}get hass(){return this._hass}getCardSize(){return 16}_discoverAll(){if(!this._hass)return{batteries:[],circuits:[],gridSensors:[],upsSensors:[],totalDraw:0};const e=this._hass.states||{},t=[],a=[],r=[],i=[];let s=0;const c=(0,n.Qn)(this._hass),f=(0,n.E3)(this._hass),v=new Set,g=[];for(const e of c)for(const t of f.get(e.floor_id)||[])g.push({floor:e,area:t});for(const e of f.get(null)||[])g.push({floor:null,area:e});for(const{floor:n,area:c}of g){const f=(0,o.d6)(this._hass,c.area_id,this._entityCache);for(const o of f){const f=o.entity_id.split(".")[0],g=e[o.entity_id];if(!g)continue;const b={entity:o,domain:f,state:g,area:c,floor:n};if((0,l.JM)(b))continue;const y=g.attributes?.device_class||"",_=o.platform||"";if("battery"===y&&o.device_id&&!v.has(o.device_id)){if(m.has(_))continue;const e=this._hass?.devices?.[o.device_id],a=(e?.name||o.entity_id||"").toLowerCase();if(h.test(a)||h.test(o.entity_id))continue;if(!u.has(_)&&!/ecoflow|river|delta|powerwall|ups/i.test(a))continue;v.add(o.device_id),t.push({entry:b,device:e,deviceId:o.device_id,area:c,floor:n});continue}if("nut"===_&&d.test(o.entity_id))i.push(b);else if("power"===y&&p.test(o.entity_id))r.push(b);else if("power"===y&&!p.test(o.entity_id)){const e=Number(g.state);isNaN(e)||(s+=e),a.push(b)}}}return a.sort((e,t)=>(Number(t.state?.state)||0)-(Number(e.state?.state)||0)),{batteries:t,circuits:a,gridSensors:r,upsSensors:i,totalDraw:s}}_getGridPower(e){const t=e.gridSensors.find(e=>"power"===e.state?.attributes?.device_class&&Number(e.state?.state)>0);return t?Number(t.state.state):e.totalDraw}_renderSystemStatus(e){const t=this._getGridPower(e);let a=0,i=0;for(const t of e.batteries){const e=Number(t.entry.state?.state);isNaN(e)||(a+=e,i++)}return i>0&&(a=Math.round(a/i)),r.qy`
      ...
      <div class="eng-section">
        ...
        <div class="eng-sources-row">
          ...
          ${e.batteries.map(e=>{const t=Number(e.entry.state?.state)||0,a=(e.device?.name||e.entry.state?.attributes?.friendly_name||"BATTERY").toUpperCase(),s=t>50?"#99ccff":t>20?"#ffcc99":"#ff5555";return r.qy`
              <div class="eng-source-card" @click=${()=>(0,i.Hv)(e.entry.entity.entity_id)}>
                ${function(e,t,a,i,s,n){const o=2*Math.PI*32,l=o*(1-Math.min(1,Math.max(0,e/100)));return r.JW`
    <svg viewBox="0 0 ${a} ${a}" width="${a}" height="${a}" class="ring-gauge" role="meter"
      ...
      <circle cx="${36}" cy="${36}" r="${32}" fill="none" stroke="${i}" stroke-width="4"
              stroke-dasharray="${o}" stroke-dashoffset="${l}"
              stroke-linecap="round" transform="rotate(-90 ${36} ${36})"
              style="transition: stroke-dashoffset 500ms ease" />
      <text x="${36}" y="${32}" text-anchor="middle" dominant-baseline="central"
            class="ring-value" style="fill:${i}">${s}</text>
      ${n?r.JW`<text x="${36}" y="${46}" text-anchor="middle" dominant-baseline="central"
            class="ring-sublabel">${n}</text>`:""}
    </svg>`}(t,0,72,s,`${t}%`,a.length>10?a.substring(0,10):a)}

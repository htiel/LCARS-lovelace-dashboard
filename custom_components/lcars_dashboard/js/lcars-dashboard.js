      ...
      `}_getDevicePanelType(e){return e.some(e=>o.has(e.domain))?h:null}_partitionDeviceEntities(e){const t=[],a=[],r=[];for(const s of e)o.has(s.domain)?t.push(s):n.has(s.domain)?a.push(s):r.push(s);return{cameras:t,sensors:a,controls:r}}_renderDevicePanel(e,t){return e===h?this._renderCameraPanel(t):""}_getSensorIndicatorColor(e){if(!e||"unavailable"===e.state||"unknown"===e.state)return"var(--lcars-tomato)";const t=e.attributes?.device_class||"",a=e.state;return"motion"===t||"occupancy"===t?"on"===a?"var(--lcars-butterscotch)":"var(--lcars-gray)":"presence"===t?"on"===a||"home"===a?"var(--lcars-gold)":"var(--lcars-gray)":["problem","safety","tamper"].includes(t)?"on"===a?"var(--lcars-tomato)":"var(--lcars-gray)":e.entity_id?.startsWith("binary_sensor.")?"on"===a?"var(--lcars-ice)":"var(--lcars-gray)":"var(--lcars-data-accent)"}_renderCameraPanel(t){const{cameras:a,sensors:r,controls:s}=this._partitionDeviceEntities(t.entities),n=t.device.name_by_user||t.device.name||"Device";return e.qy`
        <div class="lcars-device-panel" data-panel-type="camera">
          ...
          <div class="device-panel-media"
            ...
            ${a.length>0&&a[0].state.attributes?.entity_picture?e.qy`<img src="${a[0].state.attributes.entity_picture}"
                          alt="${n} camera feed" loading="lazy" />`:e.qy`<div style="display:flex;align-items:center;justify-content:center;height:100%">
                  <ha-icon icon="mdi:video-off" style="--mdc-icon-size:48px;color:var(--lcars-gray)"></ha-icon>
                </div>`}
            ${a.length>1?a.slice(1).map(({entity:t,state:a})=>e.qy`
              <img src="${a.attributes?.entity_picture||""}"
                   alt="${this._friendlyName(a,t)} camera feed"
                   loading="lazy"

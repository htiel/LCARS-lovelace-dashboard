      ...
      <div class="eng-dashboard">
        ...
        <div class="eng-summary">
          <span class="eng-summary__block">
            ...
            <span class="eng-summary__value" style="color:${this._getPowerTierColor(t.totalPowerW)}">${(0,c.ZV)(Math.round(t.totalPowerW))} W</span>
          </span>
          ${t.batteryCount>0?r.qy`
            ...
            <span class="eng-summary__block">
              ...
              <span class="eng-summary__value" style="color:${this._getBatteryColor(t.lowestBat)}">${t.lowestBat}%</span>
            </span>
          `:""}
        </div>
        ...
        <div class="device-panel-controls" aria-label="${i} controls">
          ${f?r.qy`
            <button class="device-control-btn camera-config-cta"
              ...
              aria-label="Configure in ${T(v)}">
              <ha-icon icon="mdi:cog"></ha-icon>
              <span>${v?`CONFIGURE IN ${T(v).toUpperCase()}`:"DEVICE REQUIRES SETUP"}</span>
            </button>
          `:""}
...
`;class W extends x.j{get panelType(){return"media"}get defaultPanelTitle(){return"Media"}get frameColor(){return"var(--lcars-african-violet)"}static get styles(){return[...super.styles,h.PF,h.yW,F]}_isValidArtworkUrl(e){return!!e&&(e.startsWith("/api/")||e.startsWith("/local/"))}_getMediaTransportSymbol(e){switch(e){case"playing":return"▶";case"paused":return"❚❚";default:return"■"}}_partitionMediaEntities(e){const t=new Set;for(const a of e)"media_player"===a.domain&&a.entity?.device_id&&t.add(a.entity.device_id);const a=[],r=[],i=[],s=[];for(const n of e){if("media_player"===n.domain){a.push(n);continue}const e=n.entity?.device_id;e&&t.has(e)&&("remote"!==n.domain?"binary_sensor"!==n.domain&&"camera"!==n.domain&&(o.Xt.has(n.domain)?r.push(n):i.push(n)):s.push(n))}return{player:a,sensors:r,controls:i,remotes:s}}_handleMediaService(e,t,a={}){g.e.play("mediaAction"),this.hass.callService("media_player",t,{entity_id:e,...a})}_handleVolumeChange(e,t){const a=t.currentTarget.getBoundingClientRect(),r=Math.max(0,Math.min(1,(t.clientX-a.left)/a.width));g.e.play("climateAdjust"),this._handleMediaService(e,"volume_set",{volume_level:Math.round(100*r)/100})}renderBadge(){const e=this.group?.entities?.find(e=>"media_player"===e.domain);if(!e)return r.qy``;const t=e.state?.state||"unavailable";if("unavailable"===t)return r.qy`<span style="color:var(--lcars-gray)">■ OFFLINE</span>`;const a=(0,l.uT)(t),i=this._getMediaTransportSymbol(t);return r.qy`<span style="color:${a}">${i} ${t.toUpperCase()}</span>`}renderContent(){const{player:e,sensors:t}=this._partitionMediaEntities(this.group.entities),a=this._shortDeviceName(this.group.device)||"Media";if(0===e.length)return r.qy``;const i=e.filter(e=>"playing"===e.state?.state||"paused"===e.state?.state),s=e.filter(e=>"unavailable"!==e.state?.state),n=i.length>0?s:e;if(0===n.length)return r.qy`
      ...
      `;const o=this._selectPrimary(n),c=n.filter(e=>e!==o),d=o.state,p=d?.attributes||{},u=d?.state||"unavailable",m=((0,l.uT)(u),this._getMediaTransportSymbol(u),"playing"===u),h=!(m||"paused"===u),f=p.entity_picture,v=this._isValidArtworkUrl(f),g=p.media_title||"",b=p.media_artist||"",y=p.source||"",_=null!=p.volume_level?Number(p.volume_level):0,w=p.is_volume_muted||!1,x=p.supported_features||0,$=!!(16&x),k=!!(32&x),S=!!(4&x),C=!!(32768&x),E=!!(262144&x),z=p.shuffle||!1,A=p.repeat||"off";return r.qy`
      <div class="media-content ${h?"media-idle":""}">

        <div class="media-metadata" role="list" aria-label="${a} info">
          ${y?r.qy`<div class="device-sensor-line" role="listitem"><div class="sensor-indicator" style="background:var(--lcars-african-violet)"></div><span class="sensor-label">Source</span><span class="sensor-state-value">${y}</span></div>`:""}
          <div class="media-metadata-extra" role="presentation">
          ${C?r.qy`<div class="device-sensor-line" role="listitem"><div class="sensor-indicator" style="background:${z?"var(--lcars-african-violet)":"var(--lcars-gray)"}"></div><span class="sensor-label">Shuffle</span><span class="sensor-state-value">${z?"ON":"OFF"}</span></div>`:""}
          ${E?r.qy`<div class="device-sensor-line" role="listitem"><div class="sensor-indicator" style="background:${"off"!==A?"var(--lcars-african-violet)":"var(--lcars-gray)"}"></div><span class="sensor-label">Repeat</span><span class="sensor-state-value">${A.toUpperCase()}</span></div>`:""}
        ...
        <div class="media-viewscreen ${m?"media-viewscreen-glow":""}" @click=${()=>this._handleEntityClick(o.entity.entity_id)}>
          ...
          ${h?"":r.qy`
            <div class="media-now-playing">
              ${g?r.qy`<div class="media-title">${g}</div>`:""}
              ${b?r.qy`<div class="media-artist">${b}</div>`:""}
            </div>
          `}
        </div>
        ...
        <div class="media-controls">
          <div class="media-transport" role="toolbar" aria-label="Transport controls">
            ${C?r.qy`<button class="media-transport-btn" aria-pressed="${z}" title="Shuffle" @click=${()=>this._handleMediaService(o.entity.entity_id,"shuffle_set",{shuffle:!z})}>⇄</button>`:""}
            ${$?r.qy`<button class="media-transport-btn" title="Previous" @click=${()=>this._handleMediaService(o.entity.entity_id,"media_previous_track")}>⏮</button>`:""}
            ...
            ${k?r.qy`<button class="media-transport-btn" title="Next" @click=${()=>this._handleMediaService(o.entity.entity_id,"media_next_track")}>⏭</button>`:""}
            ${E?r.qy`<button class="media-transport-btn" aria-pressed="${"off"!==A}" title="Repeat: ${A}" @click=${()=>this._handleMediaService(o.entity.entity_id,"repeat_set",{repeat:"off"===A?"all":"all"===A?"one":"off"})}>🔁</button>`:""}
          </div>
          ${S?r.qy`
            <div class="media-volume ${_>=1?"media-volume-warn":""}" aria-label="Volume: ${Math.round(100*_)}%">
              ...
              <div class="media-volume-bar" tabindex="0" role="slider"
                ...
                @keydown=${e=>{"ArrowRight"===e.key&&(e.preventDefault(),this._handleMediaService(o.entity.entity_id,"volume_set",{volume_level:Math.min(1,_+.05)})),"ArrowLeft"===e.key&&(e.preventDefault(),this._handleMediaService(o.entity.entity_id,"volume_set",{volume_level:Math.max(0,_-.05)})),"Home"===e.key&&(e.preventDefault(),this._handleMediaService(o.entity.entity_id,"volume_set",{volume_level:0})),"End"===e.key&&(e.preventDefault(),this._handleMediaService(o.entity.entity_id,"volume_set",{volume_level:1})),"PageUp"===e.key&&(e.preventDefault(),this._handleMediaService(o.entity.entity_id,"volume_set",{volume_level:Math.min(1,_+.1)})),"PageDown"===e.key&&(e.preventDefault(),this._handleMediaService(o.entity.entity_id,"volume_set",{volume_level:Math.max(0,_-.1)}))}}>
        ...
        <div class="ls-summary">
          ${null!=t.avgTemp?r.qy`
            <span class="ls-summary__block">
              ...
              <span class="ls-summary__value" style="color:${this._getComfortColor(t.avgTemp)}">${t.avgTemp}°</span>
            </span>
          `:""}
          ${null!=t.outdoor?r.qy`
            <span class="ls-summary__block">
          ...
          ${t.worstAqi>0?r.qy`
            <span class="ls-summary__block">
              ...
              <span class="ls-summary__value" style="color:${this._getAqiColor(t.worstAqi)}">${t.worstAqi} (${t.worstAqiArea.toUpperCase()})</span>
            </span>
          `:""}
          <span class="ls-summary__block">
...
`},7908(e,t,a){var r=a(7349),i=a(8851),s=a(2622),n=a(7597),o=a(6930),l=a(4867);const c="access",d="zones",p=new Set(["lock","alarm_control_panel"]),u=(new Set(["door","window","motion","occupancy","tamper","safety","smoke","gas","vibration"]),new Set(["smoke","gas","safety","tamper","vibration"])),m=new Set(["door","window"]);class h extends r.WF{static get properties(){return{hass:{type:Object},_config:{type:Object},filter:{type:String}}}constructor(){super(),this.hass=null,this._config={},this.filter="all",this._entityCache=new Map,this._onFilter=e=>{this.filter=e.detail.filter}}connectedCallback(){super.connectedCallback(),i.o6.addEventListener("lcars-tac-filter",this._onFilter)}disconnectedCallback(){super.disconnectedCallback(),i.o6.removeEventListener("lcars-tac-filter",this._onFilter)}setConfig(e){this._config=e||{}}set hass(e){const t=this._hass;this._hass=e,e&&t!==e&&(this._entityCache.clear(),this.requestUpdate("hass",t))}get hass(){return this._hass}getCardSize(){return 12}_getAreasWithTactical(){if(!this._hass)return[];const e=(0,n.Qn)(this._hass),t=(0,n.E3)(this._hass),a=[];for(const r of e){const e=t.get(r.floor_id)||[],i=[];for(const t of e){const e=this._resolveArea(t);e&&i.push(e)}i.length>0&&a.push({floor:r,areas:i})}const r=t.get(null)||[],i=[];for(const e of r){const t=this._resolveArea(e);t&&i.push(t)}return i.length>0&&a.push({floor:null,areas:i}),a}_resolveArea(e){const t=(0,o.d6)(this._hass,e.area_id,this._entityCache),a=[],r=[];for(const e of t){const t=e.entity_id.split(".")[0],i=this._hass.states?.[e.entity_id];if(!i)continue;const s={entity:e,domain:t,state:i};if(!(0,l.JM)(s)){if("camera"===t){const t=e.entity_id;/_low$|_medium$|_insecure$/.test(t)||r.push(s);continue}(0,l.XY)(s)&&a.push(s)}}const i=a.filter(e=>p.has(e.domain)),s=a.filter(e=>"binary_sensor"===e.domain&&m.has(e.state?.attributes?.device_class||"")),n=a.filter(e=>"binary_sensor"===e.domain&&u.has(e.state?.attributes?.device_class||"")),c=a.filter(e=>"binary_sensor"===e.domain&&("motion"===e.state?.attributes?.device_class||"occupancy"===e.state?.attributes?.device_class));return 0===a.length&&0===r.length?null:{area:e,entities:a,access:i,perimeter:s,safety:n,motion:c,cameras:r}}_getFiltered(e){if(this.filter===c)return e.access.length>0?e.access:null;if(this.filter===d){const t=[...e.perimeter,...e.safety,...e.motion];return t.length>0?t:null}return[...e.entities,...e.cameras]}_getGlobalSummary(e){const t={triggered:5,pending:4,armed_away:3,armed_home:2,armed_night:2,armed_vacation:2,arming:1,disarmed:0};let a="disarmed",r=0,i=0,s=0;for(const{areas:n}of e)for(const e of n){for(const r of e.access)if("alarm_control_panel"===r.domain){const e=(this._hass?.states?.[r.entity?.entity_id]||r.state)?.state||"disarmed";(t[e]||0)>(t[a]||0)&&(a=e)}for(const t of e.perimeter)r++,"on"!==(this._hass?.states?.[t.entity?.entity_id]||t.state)?.state&&i++;for(const t of e.safety)"on"===(this._hass?.states?.[t.entity?.entity_id]||t.state)?.state&&s++}return{alarmState:a,perimeterTotal:r,perimeterSecure:i,safetyAlerts:s}}_getSummaryColor(e){switch(e){case"armed_away":return"var(--lcars-sunflower, #ffcc99)";case"armed_home":case"armed_night":case"armed_vacation":return"var(--lcars-butterscotch, #ff9966)";case"triggered":case"pending":return"var(--lcars-tomato, #ff5555)";default:return"var(--lcars-ice, #99ccff)"}}render(){if(!this._hass)return r.qy``;const e=this._getAreasWithTactical(),t=this._getGlobalSummary(e),a="triggered"===t.alarmState,i=[];for(const{areas:t}of e)for(const e of t)i.push(...e.cameras);return r.qy`
      <div class="tac-dashboard ${a?"red-alert":""}">
        <!-- Summary Bar -->
        <div class="tac-summary" style="--summary-color:${this._getSummaryColor(t.alarmState)}">
          <span class="tac-summary__block">
            <span class="tac-summary__label">SHIELDS</span>
            <span class="tac-summary__value">${t.alarmState.replace(/_/g," ").toUpperCase()}</span>
          </span>
          <span class="tac-summary__block">
            <span class="tac-summary__label">PERIMETER</span>
            <span class="tac-summary__value">${t.perimeterSecure}/${t.perimeterTotal} SECURE</span>
          </span>
          <span class="tac-summary__block">
            <span class="tac-summary__label">SENSORS</span>
            <span class="tac-summary__value">${0===t.safetyAlerts?"ALL CLEAR":t.safetyAlerts+" ALERTS"}</span>
          </span>
        </div>

        <!-- Camera Grid -->
        ${i.length>0&&this.filter!==d?r.qy`
        ...
        ${e.map(({floor:e,areas:t})=>{const a=t.filter(e=>e.entities.length>0&&null!==this._getFiltered(e));return 0===a.length?r.qy``:r.qy`
            ${e?r.qy`<div class="tac-floor-header"><span class="tac-floor-name">${e.name||"FLOOR"}</span><span class="tac-floor-line"></span></div>`:""}
            ${a.map(e=>r.qy`
              <div class="tac-area-section">
                <div class="tac-area-header">
                  <span class="tac-area-name">${e.area.name}</span>
                  <span class="tac-area-line"></span>
                </div>
                <div class="tac-devices">
                  ${this._getSortedDevices(e).map(e=>this._renderDevice(e))}
                </div>
              </div>
            `)}
          `})}
        ${0===e.length?r.qy`<div class="tac-empty"><span>NO TACTICAL DEVICES DETECTED</span></div>`:""}
      </div>
    `}_getSortedDevices(e){return[...this.filter===c?e.access:this.filter===d?[...e.perimeter,...e.safety,...e.motion]:e.entities].sort((e,t)=>(this._isActive(e.domain,this._hass?.states?.[e.entity?.entity_id]||e.state)?0:1)-(this._isActive(t.domain,this._hass?.states?.[t.entity?.entity_id]||t.state)?0:1))}_renderCamera(e){const t=e.entity?.entity_id,a=this._hass?.states?.[t]||e.state,s=(a?.attributes?.friendly_name||t||"").toUpperCase(),n=a?.attributes?.entity_picture,o=t.replace("camera.","binary_sensor.").replace(/_high$/,"_motion"),l="on"===this._hass?.states?.[o]?.state;return r.qy`
      <div class="tac-camera ${l?"motion":""}"
           @click=${()=>(0,i.Hv)(t)}
           role="button" tabindex="0" aria-label="${s}">
        ${n?r.qy`<img src="${n}" alt="${s}" loading="lazy">`:r.qy`<div class="tac-camera__placeholder"><ha-icon .icon=${"mdi:camera"} style="--mdc-icon-size:32px"></ha-icon></div>`}
        <span class="tac-camera__label">${s}</span>
      </div>
    `}_renderDevice(e){const t=e.entity?.entity_id,a=this._hass?.states?.[t]||e.state,s=(a?.attributes?.friendly_name||t||"").toUpperCase(),n=a?.attributes?.device_class||"",o=this._isActive(e.domain,a);return"binary_sensor"===e.domain&&m.has(n)?r.qy`
        <button class="tac-pill ${o?"breach":"sealed"}"
                aria-label="${s} — ${o?"BREACH":"SEALED"}"
                @click=${()=>(0,i.Hv)(t)}>
          <ha-icon .icon=${this._getIcon(e.domain,n,a)} style="--mdc-icon-size:18px"></ha-icon>
          <span class="tac-pill__name">${s}</span>
          <span class="tac-pill__state">${o?"BREACH":"SEALED"}</span>
        </button>
      `:"binary_sensor"===e.domain&&u.has(n)?r.qy`
      ...
      `:"binary_sensor"!==e.domain||"motion"!==n&&"occupancy"!==n?r.qy`
      <button class="tac-pill ${o?"active":"inactive"} ${e.domain}"
              aria-label="${s} — ${(a?.state||"").toUpperCase()}"
              @click=${()=>(0,i.Hv)(t)}>
        <ha-icon .icon=${this._getIcon(e.domain,n,a)} style="--mdc-icon-size:18px"></ha-icon>
        <span class="tac-pill__name">${s}</span>
        <span class="tac-pill__state">${(a?.state||"unknown").replace(/_/g," ").toUpperCase()}</span>
      </button>
    `:r.qy`
      ...
      `}_isActive(e,t){return"lock"===e?"unlocked"===t?.state:"alarm_control_panel"===e?"disarmed"!==t?.state:"binary_sensor"===e&&"on"===t?.state}_getIcon(e,t,a){return"lock"===e?"locked"===a?.state?"mdi:lock":"mdi:lock-open":"alarm_control_panel"===e?"mdi:shield":"door"===t?"on"===a?.state?"mdi:door-open":"mdi:door-closed":"window"===t?"on"===a?.state?"mdi:window-open":"mdi:window-closed":"motion"===t||"occupancy"===t?"mdi:motion-sensor":"smoke"===t?"mdi:smoke-detector":"gas"===t?"mdi:gas-cylinder":"mdi:shield-alert"}static get styles(){return[s.Bx,r.AH`
        :host { display: block; }

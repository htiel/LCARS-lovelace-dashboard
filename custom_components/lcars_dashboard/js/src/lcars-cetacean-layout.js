/**
 * lcars-cetacean-layout.js
 *
 * LCARS frame layout for the Cetacean Ops (Pool & Spa) dashboard.
 * Sidebar: ALL / WATER BODIES / CHEMISTRY / FEATURES / POWER filter buttons.
 * Color: sky frame, african-violet sidebar.
 *
...
class LcarsCetaceanLayout extends LitElement {
  ...
  _setFilter(filter) {
    ...
    lcarsEventBus.dispatchEvent(new CustomEvent('lcars-cet-filter', { detail: { filter } }));
  }
  ...
  _toggleEditMode() {
    ...
    lcarsEventBus.dispatchEvent(new CustomEvent('lcars-cet-edit', { detail: { enabled: this._editMode } }));
  }
      ...
      <div class="lcars-frame">
        ...
        <nav class="lcars-sidebar" role="tablist" aria-label="Filter cetacean systems">
          ...
          <div class="lcars-sidebar-filters">
            ...
            <button class="sidebar-filter-btn ${this._filter === FILTER_WATER ? 'active' : ''}" role="tab" aria-selected="${this._filter === FILTER_WATER ? 'true' : 'false'}" @click=${() => this._setFilter(FILTER_WATER)}><span class="filter-label">WATER BODIES</span></button>
            <button class="sidebar-filter-btn ${this._filter === FILTER_CHEMISTRY ? 'active' : ''}" role="tab" aria-selected="${this._filter === FILTER_CHEMISTRY ? 'true' : 'false'}" @click=${() => this._setFilter(FILTER_CHEMISTRY)}><span class="filter-label">CHEMISTRY</span></button>
            <button class="sidebar-filter-btn ${this._filter === FILTER_FEATURES ? 'active' : ''}" role="tab" aria-selected="${this._filter === FILTER_FEATURES ? 'true' : 'false'}" @click=${() => this._setFilter(FILTER_FEATURES)}><span class="filter-label">FEATURES</span></button>
            <button class="sidebar-filter-btn ${this._filter === FILTER_POWER ? 'active' : ''}" role="tab" aria-selected="${this._filter === FILTER_POWER ? 'true' : 'false'}" @click=${() => this._setFilter(FILTER_POWER)}><span class="filter-label">POWER</span></button>
          </div>
...
ready.then(() => { if (!customElements.get('lcars-cetacean-layout')) { customElements.define('lcars-cetacean-layout', LcarsCetaceanLayout); } });

...
`;
  ...
  class LcarsEditDeviceButtonCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'lcars_dashboard/device_button/set', ...this._getFormData() });
  ...
  if (!customElements.get('lcars-edit-device-button-card')) {
    customElements.define('lcars-edit-device-button-card', LcarsEditDeviceButtonCard);
  }
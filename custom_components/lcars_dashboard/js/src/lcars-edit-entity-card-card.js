...
`;
  ...
  class LcarsEditEntityCardCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'lcars_dashboard/entity_card/set', ...this._getFormData() });
  ...
  if (!customElements.get('lcars-edit-entity-card-card')) {
    customElements.define('lcars-edit-entity-card-card', LcarsEditEntityCardCard);
  }
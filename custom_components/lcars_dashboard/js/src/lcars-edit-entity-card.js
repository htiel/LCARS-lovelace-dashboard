...
`;
  ...
  class LcarsEditEntityCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'lcars_dashboard/entity/set', ...this._getFormData() });
  ...
  if (!customElements.get('lcars-edit-entity-card')) {
    customElements.define('lcars-edit-entity-card', LcarsEditEntityCard);
  }
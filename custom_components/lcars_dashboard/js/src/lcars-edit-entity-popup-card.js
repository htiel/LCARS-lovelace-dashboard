...
`;
  ...
  class LcarsEditEntityPopupCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'lcars_dashboard/entity_popup/set', ...this._getFormData() });
  ...
  if (!customElements.get('lcars-edit-entity-popup-card')) {
    customElements.define('lcars-edit-entity-popup-card', LcarsEditEntityPopupCard);
  }
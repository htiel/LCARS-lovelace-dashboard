...
`;
  ...
  class LcarsEditHomepageHeaderCard extends LitElement {
    ...
    async _save() {
      ...
      try {
        await this._hass.callWS({ type: 'lcars_dashboard/homepage_header/set', ...this._getFormData() });
  ...
  if (!customElements.get('lcars-edit-homepage-header-card')) {
    defineLcars('lcars-edit-homepage-header-card', LcarsEditHomepageHeaderCard);
  }
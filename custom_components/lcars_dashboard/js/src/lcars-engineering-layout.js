/**
 * lcars-engineering-layout.js
 *
 * LCARS frame layout for the Engineering (Power) dashboard.
 * Sidebar: ALL / STORAGE / CIRCUITS filter buttons.
 * Color: butterscotch frame, african-violet sidebar.
 *
 * v5.0.0 — 5X-2.3 Power Dashboard
 */
...
class LcarsEngineeringLayout extends LitElement {
  ...
  _setFilter(filter) {
    ...
    lcarsEventBus.dispatchEvent(new CustomEvent('lcars-eng-filter', { detail: { filter } }));
  }
...
ready.then(() => { if (!customElements.get('lcars-engineering-layout')) { customElements.define('lcars-engineering-layout', LcarsEngineeringLayout); } });

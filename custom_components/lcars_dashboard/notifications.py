...
EVENT_lcars_dashboard_NOTIFICATIONS_UPDATED = "lcars_dashboard_notifications_updated"
...
def websocket_get_notifications(
    ...
    """Return a list of lcars_dashboard_notifications."""
...
def notifications(hass, name):
    ...
    """Set up the LCARS Dashboard notification component."""
    
    lcars_dashboard_notifications: MutableMapping[str, MutableMapping] = OrderedDict()
    hass.data[DOMAIN]["notifications"] = lcars_dashboard_notifications
    ...
    def create_service(call):
        ...
        lcars_dashboard_notifications[entity_id] = {
            ATTR_MESSAGE: message,
            ATTR_NOTIFICATION_ID: notification_id,
    ...
    def dismiss_service(call):
        ...
        if entity_id not in lcars_dashboard_notifications:
            return

        hass.states.async_remove(entity_id)

        del lcars_dashboard_notifications[entity_id]
        hass.bus.async_fire(EVENT_lcars_dashboard_NOTIFICATIONS_UPDATED)
    ...
    def mark_read_service(call):
        ...
        if entity_id not in lcars_dashboard_notifications:
            _LOGGER.error(
                "Marking LCARS Dashboard_notification read failed: "
                "Notification ID %s not found.",
                notification_id,
            )
            return

        lcars_dashboard_notifications[entity_id][ATTR_STATUS] = STATUS_READ
        hass.bus.async_fire(EVENT_lcars_dashboard_NOTIFICATIONS_UPDATED)

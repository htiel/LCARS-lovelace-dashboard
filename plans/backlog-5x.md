...
#### 5X-F37 · Moonraker (Ender 3 V3 KE / Klipper) — Extend Fabrication Panel With Second Printer Platform — `TODO` · Priority: MEDIUM · Size: M
Captain's Ender 3 V3 KE is now reachable from HA via the Moonraker HACS integration (Creality `Ender-3_V3_KE_Annex` repo → enable root → SSH → install Fluidd → install Moonraker HACS integration → connect; print-status notifications per the [r/Ender3V3KE Reddit comment](https://www.reddit.com/r/Ender3V3KE/comments/1acfwp1/comment/kju4irm/); reference video <https://www.youtube.com/watch?v=l2JCWSBQczg>). **Captain setup runbook**: [localinfo/ender3-v3-ke-moonraker-setup.md](../localinfo/ender3-v3-ke-moonraker-setup.md) (gitignored). Extend `FABRICATION_PLATFORMS` in [custom_components/lcars_dashboard/js/src/lcars-engineering-card.js](../custom_components/lcars_dashboard/js/src/lcars-engineering-card.js#L27) to include `'moonraker'`, map Moonraker entity vocabulary (`*_progress`, `*_current_print_state`, `*_extruder_temperature`, `*_bed_temperature`, `*_print_duration`, etc.) onto the existing Bambu `_renderFabPrinter` anatomy in [lcars-engineering-card.js](../custom_components/lcars_dashboard/js/src/lcars-engineering-card.js#L346), and update [specs/LCARS-FABRICATION-PANEL-SPEC.md](../specs/LCARS-FABRICATION-PANEL-SPEC.md) §2 (discovery — slug derivation differs: Moonraker uses friendly-name-derived prefixes, not model+serial) and §3 (status mapping — Klipper states are `standby` / `printing` / `paused` / `complete` / `error` / `cancelled`, no AMS sub-devices). Office Habitat tile should aggregate across both printers. HACS pin: `Moonraker` (`marcolivierarsenault/moonraker-home-assistant`).
